 ...
 *
 * Read client/src/App.tsx's `AdminRoute` before writing this: it does NOT
 * silently redirect a non-admin away. For an authenticated non-admin it
 * renders an explicit `ForbiddenPage` component in place, at the same
 * /admin URL — a real "403" screen (eyebrow text "403", heading "Backstage
 * only.") rather than a bounce to "/" or "/login". This is a genuine
 * client-side UX decision on top of the server, which independently
...
test('a signed-in customer visiting /admin sees an explicit 403 screen, not a silent redirect', async ({ page }) => {
  ...
  // The same guard covers the whole /admin/* subtree, not just the index route.
